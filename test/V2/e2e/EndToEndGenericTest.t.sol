...
contract EndToEndV2GenericTest is Helper {
    ...
    function _setupChainlinkV2(address sequencer, address oracleFeed) internal {
        ...
        (, , , uint256 updatedAt, ) = IPriceFeedAdapter(oracleFeed)
            .latestRoundData();
        // depeg runs at begin + 1 hours - avoids TIME_OUT reverts from Chainlink
        begin = uint40(updatedAt);
        end = uint40(updatedAt + 3 days);
    ...
    function test_GenericDepegChainlinkVol() public {
        _setupChainlinkV2(ARBITRUM_SEQUENCER_GOERLI, ETH_VOL_CHAINLINK);
        vm.startPrank(USER);
        configureDepegState(
            depegPremium,
            depegCollateral,
